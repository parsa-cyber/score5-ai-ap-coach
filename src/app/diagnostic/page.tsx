...
import { addAttempt, getProfile } from "@/lib/storage";
      ...
      <main>
        ...
        <section className="mx-auto max-w-5xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="mb-6">
            ...
            <h1 className="mt-2 text-4xl font-black tracking-tight text-slate-950">Your {courseInfo.shortName} starting point</h1>
          </div>
          <div className="grid gap-5 md:grid-cols-3">
            <StatCard label="Estimated AP score" value={estimate.score} helper="Based on this diagnostic" />
          ...
          <Card className="mt-5">
            <h2 className="text-2xl font-black">Weakest areas to fix first</h2>
            <div className="mt-4 grid gap-3">
              {weak.map((unit, i) => (
                <div key={unit} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4">
                  ...
                  <span className="text-sm font-bold text-brand-700">recommended</span>
                </div>
              ))}
            </div>
            <div className="mt-6 flex flex-col gap-3 sm:flex-row">
              ...
              <Link href="/practice" className="rounded-full border border-slate-300 bg-white px-5 py-3 text-center font-bold text-slate-900">Practice weak topics</Link>
            </div>
          </Card>
        </section>
      </main>
